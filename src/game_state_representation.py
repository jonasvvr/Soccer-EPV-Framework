...
def get_sparse_matrices(row, attacking_team, field_dimen):
    loc_att = np.zeros(field_dimen)
    loc_def = np.zeros(field_dimen)
    ...
    for player_data in all_player_data:
        ...
        if player_data['Object type'] in is_attacking:
            # Location
            loc_att[x, y] = 1  # accurate location
...
def get_game_state_representation(row, attacking_team, field_dimen):
    loc_att, loc_def, vx_att, vx_def, vy_att, vy_def, cos_angle, sin_angle = get_sparse_matrices(
        row, attacking_team,
        field_dimen)
    dist_b, dist_g, angle_ball_sin, angle_ball_cos, angle_goal_sin, angle_goal_cos, angle_goal_rad = get_dense_matrices(
        row, field_dimen)

    return [loc_att, loc_def, vx_att, vx_def, vy_att, vy_def, dist_b, dist_g, angle_ball_sin,
