...
def plot_events( events, figax=None, field_dimen = (106.0,68.0), indicators = ['Marker','Arrow'], color='r', marker_style = 'o', alpha = 0.5, annotate=False):
    """ plot_events( events )
    ...
    for i,row in events.iterrows():
        if row['Type'] == 'SET PIECE': 
            continue
        if np.isnan(row['Start X']) | np.isnan(row['Start Y']) | np.isnan(row['End X']) | np.isnan(row['End Y']): 
            continue
